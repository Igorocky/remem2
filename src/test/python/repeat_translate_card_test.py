...
{prompt}(press Enter to go to the next task){end}""",
    ...
    def test_press_enter_as_the_first_input(self) -> None:
        # given
        task_id = 971
        self._init_mocks()
        card = make_simple_card()
...
""",
        ...
        # when press Enter instead of the first input
        state = process_user_input(state, '')
        ...
        # when render the sate after the first input (which was the Enter)
        render_state(self.console, state)
...
{prompt}Enter mark [1]: {end}""",
        ...
        # when press Enter to set mark 1
        state = process_user_input(state, '')
...
{prompt}Enter mark [1]: {end}""",
        ...
        # when render the state after executing the command
