...
def _select_options(c: Console, options: list[str], single: bool) -> list[int]:
    filt: list[str] = []
...
def select_single_option(c: Console, options: list[str]) -> int | None:
    idxs = _select_options(c, options, single=True)
    if len(idxs) == 0:
        return None
    return idxs[0]


def select_multiple_options(c: Console, options: list[str]) -> list[int]:
    return _select_options(c, options, single=False)
